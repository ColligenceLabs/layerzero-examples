...
let FACTORY_ABI = ["function getPool(uint256) view returns(address)"]

let POOL_ABI = ["function token() view returns(address)", "function convertRate() view returns(uint256)"]
...
module.exports = async function (taskArgs, hre) {
    ...
    const srcAddress = hre.ethers.utils.solidityPack(["address", "address"], [STG_BRIDGE[srcNetwork], STG_BRIDGE[hre.network.name]])

    const decodedPayload = ethers.utils.defaultAbiCoder.decode(SWAP_REMOTE_UA_PAYLOAD_ENCODING, uaPayload)
    ...
    const sgReceiveCallData = interfaceStargateReceiver.encodeFunctionData("sgReceive", [
        srcChainId,
        callDataSrcAddress,
        nonce,
        token,
